(function(){
  ...
  function bindUsersPanel(){
    ...
    if(name) name.oninput = function(){
      if(login && !login.dataset.touched) login.value = idify(name.value);
    };
  ...
  async function loadUsers(){
    try {
      ...
      status(e.message || 'Erro ao carregar usuários.', 'err');
    }
  }
  ...
  function renderUserList(){
    ...
    if(!usersCache.length){
      list.innerHTML = '<p class="hint">Nenhuma vendedora com usuário criado ainda.</p>';
      return;
    }
  ...
  async function toggleUser(id){
    ...
    if(!u) return;
    try {
      await api('/api/admin/users', { method:'POST', body:JSON.stringify({ id:u.id, username:u.username, name:u.name, sellerId:u.sellerId, active:u.active === false }) });
      await loadUsers();
      status('Status atualizado.', 'ok');
  ...
  async function resetPassword(id){
    ...
    if(!pass) return;
    try {
      await api('/api/admin/users', { method:'POST', body:JSON.stringify({ id:u.id, username:u.username, name:u.name, sellerId:u.sellerId, active:u.active !== false, password:pass }) });
      await loadUsers();
      status('Senha alterada.', 'ok');
