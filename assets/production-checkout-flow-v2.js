(function(){
  'use strict';
  if(window.__ARMAZEM_PRODUCTION_CHECKOUT_FLOW_V2__)return;
  window.__ARMAZEM_PRODUCTION_CHECKOUT_FLOW_V2__='2026-08-04.2';
  ...
  document.documentElement.dataset.productionCheckoutFlow='review-customer-register-v2';
  ...
  function interceptCheckout(event){
    ...
    if(anchor.classList.contains('disabled'))return notify('Revise o carrinho antes de enviar.');
    var rule=typeof cartRule==='function'?cartRule():{ok:false,msg:'Revise o carrinho.'};
    if(!rule||rule.ok!==true)return notify(rule&&rule.msg||'Revise o carrinho antes de enviar.');
    var snapshot=createSnapshot();
    if(!snapshot.items.length)return notify('Seu carrinho está vazio.');
    submission={snapshot:snapshot,customer:readDraft(),idempotencyKey:'',accepted:null,whatsappUrl:'',intent:null};
    showReview();
  }
  ...
  function createSnapshot(){
    ...
    var mapped=source.map(function(item,index){
  ...
  function showReview(){
    step='review';ensureOverlay();
    var snapshot=submission.snapshot;
    overlay.innerHTML='<section class="checkoutFlowCard checkoutFlowReview" role="dialog" aria-modal="true" aria-labelledby="checkoutFlowTitle">'+progress(1)+
      '<header class="checkoutFlowHeader"><div><span class="checkoutFlowEyebrow">Antes de registrar</span><h2 id="checkoutFlowTitle">Confira suas artes</h2><p>Veja cada arte, produto e quantidade. Você ainda pode voltar ao carrinho para alterar.</p></div><button type="button" class="checkoutFlowClose" data-close aria-label="Fechar">×</button></header>'+
      '<div class="checkoutFlowItems">'+snapshot.items.map(reviewItem).join('')+'</div>'+totalsMarkup(snapshot)+
      '<footer class="checkoutFlowActions"><button type="button" class="checkoutFlowSecondary" data-close>Voltar ao carrinho</button><button type="button" class="checkoutFlowPrimary" data-next>Continuar</button></footer></section>';
    bindCommon();overlay.querySelector('[data-next]').onclick=showCustomer;focusHeading();
  }

  function reviewItem(item){
    var meta=[item.productName,item.theme,item.measurement,item.observation].filter(Boolean);
    return '<article class="checkoutFlowItem"><div class="checkoutFlowThumb">'+(item.image?'<img src="'+escapeHtml(item.image)+'" alt="Arte código '+escapeHtml(item.code)+'" loading="lazy">':'<span>Sem imagem</span>')+'</div><div class="checkoutFlowItemInfo"><div class="checkoutFlowItemTop"><strong>Arte #'+escapeHtml(item.code)+'</strong><b>'+item.quantity+' un.</b></div><div class="checkoutFlowMeta">'+meta.map(function(value){return '<span>'+escapeHtml(value)+'</span>';}).join('')+'</div></div></article>';
  }

  function totalsMarkup(snapshot){
    var discountValue=Number(snapshot.discount||0);
    var units=snapshot.items.reduce(function(sum,item){return sum+item.quantity;},0);
    return '<aside class="checkoutFlowTotals"><div><span>'+snapshot.items.length+' arte(s) selecionada(s)</span><strong>'+units+' unidade(s)</strong></div>'+(discountValue>0?'<div><span>Desconto</span><strong>- '+money(discountValue)+'</strong></div>':'')+'<div class="checkoutFlowGrand"><span>Total</span><strong>'+money(snapshot.total)+'</strong></div></aside>';
  }

  function showCustomer(){
    step='customer';ensureOverlay();
    var draft=submission.customer||{};
    overlay.innerHTML='<form class="checkoutFlowCard checkoutFlowCustomer" role="dialog" aria-modal="true" aria-labelledby="checkoutFlowTitle">'+progress(2)+
      ...
      '<div class="checkoutFlowFields"><label><span>Seu nome</span><input name="name" maxlength="160" required autocomplete="name" placeholder="Digite seu nome completo"></label><label><span>WhatsApp com DDD</span><input name="whatsapp" maxlength="20" required inputmode="tel" autocomplete="tel" placeholder="(81) 99999-9999"></label></div><p class="checkoutFlowError" data-error role="alert"></p>'+
      '<footer class="checkoutFlowActions"><button type="button" class="checkoutFlowSecondary" data-back>Voltar</button><button type="submit" class="checkoutFlowPrimary">Registrar pedido</button></footer></form>';
    bindCommon();
  ...
  async function beginRegistration(event){
    ...
    submission.customer={name:name,whatsapp:whatsapp};writeDraft(submission.customer);
    submission.intent=createIntent(submission.snapshot,submission.customer);
    try{submission.idempotencyKey=await idempotency(submission.intent);}catch(_){error.textContent='Seu navegador não conseguiu preparar o pedido. Recarregue a página e tente novamente.';return;}
  ...
  function createIntent(snapshot,customer){
    return {seller:{id:snapshot.seller.id,label:snapshot.seller.label},customer:{name:customer.name,whatsapp:customer.whatsapp},items:snapshot.items.map(function(item){return {driveFileId:item.driveFileId,productKey:item.productKey,quantity:item.quantity};})};
  }
  ...
  async function submitIntent(){
    ...
    try{
      var response=await fetch('/api/orders-v2',{method:'POST',credentials:'same-origin',headers:{Accept:'application/json','Content-Type':'application/json','Idempotency-Key':submission.idempotencyKey},body:JSON.stringify(submission.intent)});
      var data=await response.json().catch(function(){return {};});
      if(!response.ok||data.ok!==true||!data.orderNumber){var failure=new Error(data.error||'ORDER_SAVE_FAILED');failure.status=response.status;throw failure;}
  ...
  function showRegistering(title,text){
    step='registering';ensureOverlay();
    overlay.innerHTML='<section class="checkoutFlowCard checkoutFlowStatus" role="dialog" aria-modal="true" aria-labelledby="checkoutFlowTitle" aria-live="polite">'+progress(3)+'<div class="checkoutFlowSpinner" aria-hidden="true"><i></i><i></i><i></i></div><span class="checkoutFlowEyebrow">Registro seguro</span><h2 id="checkoutFlowTitle" data-status-title>'+escapeHtml(title)+'</h2><p data-status-text>'+escapeHtml(text)+'</p><div class="checkoutFlowStatusBar"><span></span></div><small>Uma nova tentativa usará a mesma identificação para recuperar o pedido sem duplicá-lo.</small></section>';
    focusHeading();
  }

  function updateRegistering(title,text){
    if(!overlay||step!=='registering')return;
    var titleNode=overlay.querySelector('[data-status-title]');var textNode=overlay.querySelector('[data-status-text]');
    if(titleNode)titleNode.textContent=title;if(textNode)textNode.textContent=text;
  }

  function showSuccess(){
    step='success';ensureOverlay();
    var number=submission.accepted.orderNumber;
    overlay.innerHTML='<section class="checkoutFlowCard checkoutFlowStatus checkoutFlowSuccess" role="dialog" aria-modal="true" aria-labelledby="checkoutFlowTitle">'+progress(3)+'<div class="checkoutFlowSuccessIcon" aria-hidden="true">✓</div><span class="checkoutFlowEyebrow">Tudo certo</span><h2 id="checkoutFlowTitle">Pedido registrado</h2><p>Seu pedido <strong>'+escapeHtml(number)+'</strong> já foi salvo. Agora abra o WhatsApp para falar com a vendedora.</p><div class="checkoutFlowOrderNumber"><span>Número do pedido</span><strong>'+escapeHtml(number)+'</strong></div><footer class="checkoutFlowActions checkoutFlowActionsStack"><button type="button" class="checkoutFlowWhatsapp" data-open-whatsapp>Abrir WhatsApp</button><button type="button" class="checkoutFlowSecondary" data-close>Fechar</button></footer><small>Se o WhatsApp não abrir, o pedido continua registrado e este botão pode ser usado novamente.</small></section>';
  ...
  function openAcceptedWhatsapp(){
    ...
    if(!validWhatsappUrl(url))return showPostAcceptanceFailure();
    var opened=null;
    try{opened=window.open(url,'_blank');if(opened)opened.opener=null;}catch(_){ }
  ...
  function showPostAcceptanceFailure(){
    ...
    step='success';ensureOverlay();
    var number=submission.accepted.orderNumber;
    overlay.innerHTML='<section class="checkoutFlowCard checkoutFlowStatus" role="dialog" aria-modal="true" aria-labelledby="checkoutFlowTitle"><div class="checkoutFlowWarningIcon" aria-hidden="true">!</div><span class="checkoutFlowEyebrow">Pedido salvo</span><h2 id="checkoutFlowTitle">O WhatsApp não abriu</h2><p>O pedido <strong>'+escapeHtml(number)+'</strong> está registrado. Nenhum novo pedido será criado ao tentar abrir o WhatsApp novamente.</p><footer class="checkoutFlowActions checkoutFlowActionsStack"><button type="button" class="checkoutFlowWhatsapp" data-open-whatsapp>Tentar abrir novamente</button><button type="button" class="checkoutFlowSecondary" data-close>Fechar</button></footer></section>';
    bindCommon();overlay.querySelector('[data-open-whatsapp]').onclick=openAcceptedWhatsapp;focusHeading();
  }

  function showFailure(error){
    step='failure';ensureOverlay();
    var message=checkoutMessage(error&&error.message,error&&error.status);
    overlay.innerHTML='<section class="checkoutFlowCard checkoutFlowStatus checkoutFlowFailure" role="dialog" aria-modal="true" aria-labelledby="checkoutFlowTitle"><div class="checkoutFlowWarningIcon" aria-hidden="true">!</div><span class="checkoutFlowEyebrow">Não concluído</span><h2 id="checkoutFlowTitle">Não conseguimos confirmar o registro</h2><p>'+escapeHtml(message)+'</p><div class="checkoutFlowHelp">Sua seleção e seus dados foram mantidos. Ao tentar novamente, usaremos a mesma chave de segurança para recuperar o pedido caso ele já tenha sido salvo.</div><footer class="checkoutFlowActions"><button type="button" class="checkoutFlowSecondary" data-back>Revisar dados</button><button type="button" class="checkoutFlowPrimary" data-retry>Tentar novamente</button></footer></section>';
    bindCommon();overlay.querySelector('[data-back]').onclick=showCustomer;overlay.querySelector('[data-retry]').onclick=function(){showRegistering('Verificando a tentativa anterior','Vamos recuperar o pedido se ele já tiver sido salvo, sem criar outro.');submitIntent();};focusHeading();
  }

  function createWhatsappUrl(snapshot,orderNumber){
    var phone=digits(snapshot.seller&&snapshot.seller.phone);if(phone.length<10)return '';
    var base=typeof waMsg==='function'?String(waMsg()||''):'Olá, gostaria de finalizar meu pedido.';
    return 'https://wa.me/'+phone+'?text='+encodeURIComponent('Pedido: '+orderNumber+'\n\n'+base);
  }

  async function idempotency(intent){
    var canonical=JSON.stringify({day:new Date().toISOString().slice(0,10),seller:intent.seller,customer:intent.customer,items:intent.items.slice().sort(function(a,b){return (a.driveFileId+':'+a.productKey).localeCompare(b.driveFileId+':'+b.productKey);})});
  ...
  function persistAccepted(orderNumber,url,key){if(!validWhatsappUrl(url))return;try{sessionStorage.setItem(ACCEPTED_ORDER,JSON.stringify({orderNumber:orderNumber,url:url,key:key,createdAt:Date.now()}));}catch(_){ }}
  function readAcceptedUrl(){try{var value=JSON.parse(sessionStorage.getItem(ACCEPTED_ORDER)||'null');if(!value||Date.now()-Number(value.createdAt||0)>ACCEPTED_TTL||!validWhatsappUrl(value.url)){sessionStorage.removeItem(ACCEPTED_ORDER);return '';}return value.url;}catch(_){return '';}}
  function validWhatsappUrl(value){try{var url=new URL(String(value||''));return url.protocol==='https:'&&url.hostname==='wa.me'&&/^\/\d{10,20}$/.test(url.pathname)&&url.searchParams.has('text');}catch(_){return false;}}

  function ensureOverlay(){if(!overlay){overlay=document.createElement('div');overlay.className='checkoutFlowOverlay';document.body.appendChild(overlay);}overlay.classList.add('show');document.documentElement.classList.add('checkoutFlowOpen');}
  function bindCommon(){overlay.querySelectorAll('[data-close]').forEach(function(button){button.onclick=closeFlow;});overlay.onclick=function(event){if(event.target===overlay&&step!=='registering')closeFlow();};}
  ...
  function progress(active){return '<nav class="checkoutFlowProgress" aria-label="Etapas da finalização"><span class="'+(active>=1?'active':'')+'"><i>1</i>Conferir</span><b></b><span class="'+(active>=2?'active':'')+'"><i>2</i>Seus dados</span><b></b><span class="'+(active>=3?'active':'')+'"><i>3</i>Registrar</span></nav>';}

  function measurementText(item,details){
    var explicit=details.measurements||item&&item.measurements||details.medidas;if(typeof explicit==='string')return clean(explicit);
    var values=[];if(details.diameter)values.push('Diâmetro '+clean(details.diameter)+' cm');if(details.width&&details.height)values.push(clean(details.width)+' × '+clean(details.height)+' '+clean(details.unit||'cm'));if(details.size)values.push('Tamanho '+clean(details.size));if(details.unknown)values.push('Medida a confirmar');return values.join(' · ');
  }
  ...
  function checkoutMessage(code,status){var messages={ARTE_NAO_ENCONTRADA:'Uma arte do carrinho não está mais disponível. Volte ao carrinho, remova essa arte e escolha outra.',ARTE_PRODUTO_INCOMPATIVEL:'Uma arte não corresponde ao produto indicado. Volte ao carrinho e selecione novamente.',PRODUTO_INDISPONIVEL:'Um produto do carrinho está temporariamente indisponível.',QUANTIDADE_BOLINHAS_INVALIDA:'Revise a quantidade de Bolinhas antes de registrar.',QUANTIDADE_PAINEL_150_INVALIDA:'Revise a quantidade do Painel 150 antes de registrar.',WHATSAPP_CLIENTE_INVALIDO:'Informe um WhatsApp válido com DDD.',NOME_CLIENTE_OBRIGATORIO:'Informe seu nome.',VENDEDORA_OBRIGATORIA:'Escolha uma vendedora.',CARRINHO_VAZIO_OU_INVALIDO:'O carrinho não contém itens válidos para registro.'};if(messages[code])return messages[code];if(status===429)return 'Muitas tentativas foram feitas em pouco tempo. Aguarde um instante e tente novamente.';if(status>=500||!status)return 'Houve uma falha de conexão ou do servidor. Tente novamente para recuperar o pedido com segurança.';return 'Revise os dados e tente registrar novamente.';}
