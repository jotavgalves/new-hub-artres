(function(){
  ...
  function drawPage(order,page,total,chunk,imgs,logo,names){
    var cmd='';cmd+=R(0,0,595,842,[255,250,246]);cmd+=R(26,716,543,92,[255,255,255],[240,226,230]);
    if(logo)cmd+=IM(names.logo,40,752,120,36);else cmd+=T('ARMAZEM',40,770,18,1,[217,54,107]);
    var c=cust(order),v=sell(order);cmd+=T('Resumo visual do pedido',180,786,8,1,[217,54,107]);cmd+=T(ono(order),180,762,20,1,[34,33,36]);cmd+=T(date(order&&order.createdAt),180,744,8,0,[111,104,114]);
    cmd+=T('Cliente',380,788,7,1,[141,133,144]);cmd+=T(cut(c.name||'Nao informado',28),380,771,10,1);cmd+=T('WhatsApp: '+cut(c.whatsapp||c.phone||'--',22),380,754,8,0,[111,104,114]);cmd+=T('Vend.: '+cut(v.label||v.name||v.id||'--',18),380,738,8,0,[111,104,114]);
    ...
    var xs=[34,304],ys=[568,426,284,142],cw=257,ch=124,iw=237,ih=80;
    chunk.forEach(function(it,i){var x=xs[i%2],y=ys[Math.floor(i/2)],nm=names.imgs[i];cmd+=R(x,y,cw,ch,[255,255,255],[240,226,230]);if(imgs[i])cmd+=IM(nm,x+10,y+36,iw,ih);else{cmd+=R(x+10,y+36,iw,ih,[255,241,246]);cmd+=T('#'+(code(it)||'--'),x+cw/2,y+76,22,1,[217,54,107],'center')}cmd+=T('#'+(code(it)||'--'),x+10,y+21,13,1);cmd+=T(qty(it)+' un.',x+cw-12,y+21,8,1,[111,104,114],'right');cmd+=T(cut(prod(it),25),x+10,y+9,8,1,[111,104,114]);cmd+=T(cut(theme(it),22),x+cw-12,y+9,8,0,[111,104,114],'right')});
    return cmd;
  }
