(function(){
  ...
  window.__ARMAZEM_VENDOR_PANEL_VERSION__ = '6';
  ...
  function confirmHref(o){var t=tools();return t&&t.confirmHref?t.confirmHref(o):wa((o.customer||{}).whatsapp||(o.customer||{}).phone)}
  function findOrder(key){key=String(key||'');return orders.find(function(o){return String(o.id||'')===key||String(orderNo(o))===key})}
  function openOrderPdf(key){var o=findOrder(key), t=tools();if(!o)return;if(!t||!t.openPdf){alert('Ferramenta de PDF ainda não carregou. Tente novamente.');return}t.openPdf(o)}
  ...
  async function loadCustomers(force,silent){
    ...
    try{
      ...
      customersSig=sigCustomers(customers);
  ...
  function orderCard(o){
    var c=o.customer||{}, phone=c.whatsapp||c.phone||'', key=String(o.id||orderNo(o));
    var items=(o.items||[]).slice(0,20).map(function(i){return '#'+esc(i.code)+' ('+esc(i.qty||1)+'x)'}).join(' · ');
    return '<article class="item orderCardV2"><div class="itemHead"><div><span class="orderNumberPill">'+esc(orderNo(o))+'</span><p class="hint">'+esc(date(o.createdAt))+' · '+esc(o.qty||0)+' item(ns)</p></div><div class="actions"><a class="btn green" href="'+esc(confirmHref(o))+'" target="_blank" rel="noopener">Confirmar no WhatsApp</a><button class="btn secondary" type="button" data-vendor-order-pdf="'+esc(key)+'">PDF do pedido</button><a class="btn secondary" href="'+esc(wa(phone))+'" target="_blank" rel="noopener">Abrir conversa</a></div></div><div class="orderCustomer"><b>Cliente:</b> '+esc(c.name||'Não informado')+' · <b>WhatsApp:</b> '+esc(phone||'Não informado')+'</div><p class="hint"><b>Status:</b> '+esc(o.status||'Novo')+' · <b>Total:</b> '+money(o.totals&&o.totals.net)+'</p><p class="hint">'+items+'</p></article>';
  }
  ...
  document.addEventListener('click',function(e){if(!user||user.role!=='vendedora')return;var pdf=e.target&&e.target.closest&&e.target.closest('[data-vendor-order-pdf]');if(pdf){e.preventDefault();openOrderPdf(pdf.getAttribute('data-vendor-order-pdf'));return}var vc=e.target&&e.target.closest&&e.target.closest('#vendorClientes');if(vc){e.preventDefault();e.stopPropagation();renderClientes();return}var vs=e.target&&e.target.closest&&e.target.closest('#vendorSolicitacoes');if(vs){e.preventDefault();e.stopPropagation();renderSolicitacoes();return}},true);
