...
function applyCampaignLogic(html, activeDiscount, discountPercent, discountFactor) {
  ...
  out = out
    ...
    .replace('${cartQty()?`<div class="discountCard"><span>Pedido organizado</span><strong>Você economiza ${money(discount())}</strong><small>O desconto já entra automaticamente no total antes de enviar para a vendedora. Quanto mais você escolhe por aqui, mais fácil fica finalizar seu pedido.</small></div>`:""}', '${cartQty()?campaignCardHtml():""}')
    ...
    .replace('<div class="totalLine"><span>Pedido organizado</span><strong>- ${money(discount())}</strong></div>', '${campaignActive()?`<div class="totalLine"><span>Desconto por aqui ${campaignPercent()}%</span><strong>- ${money(discount())}</strong></div>`:""}')
...
const CATALOG_CACHE_BUST_SCRIPT = '<script src="/assets/catalog-cache-bust.js?v=1"></script>';
