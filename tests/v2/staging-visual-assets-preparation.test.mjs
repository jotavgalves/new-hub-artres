...
test('copia design e assets e injeta contexto e WhatsApp antes do bridge somente no staging', async t => {
...
test('falha fechada sem body ou quando um dos assets de checkout já existe na origem', async t => {
  ...
  const duplicatedWhatsapp = await fixture('<html><body><script src="./assets/v2-checkout-whatsapp.js"></script></body></html>');
  t.after(duplicatedWhatsapp.cleanup);
  await assert.rejects(
    prepareSiteV2StaticAssets(duplicatedWhatsapp),
    error => error.code === 'STAGING_CHECKOUT_BRIDGE_ALREADY_PRESENT'
  );
});
